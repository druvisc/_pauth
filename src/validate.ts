...
export class Validate extends Singleton {
  ...
  public static Rule(rule: Rule, context: Settings): Error[] {
    const tag: string = `${Validate.Tag}.Rule()`;
    const ruleErrors: Error[] = [];
    return ruleErrors;
  }

  public static Target(element: Rule | Policy | PolicySet, context: Settings): Error[] {
    const tag: string = `${Validate.Tag}.Target()`;
    const targetErrors: Error[] = [];
    return targetErrors;
  }

  public static Policy(policy: Policy, context: Settings): Error[] {
    const tag: string = `${Validate.Tag}.Policy()`;
    const policyErrors: Error[] = [];
    return policyErrors;
  }

  public static PolicySet(policySet: PolicySet, context: Settings): Error[] {
